...
The backend's request/response contract is auto-generated by FastAPI from
its code, so it is the single source of truth for what this frontend sends
and renders — browse it live:

- **Swagger UI:** https://life-policy-pilot-backend-1.onrender.com/docs
- **ReDoc:** https://life-policy-pilot-backend-1.onrender.com/redoc
- **OpenAPI schema:** https://life-policy-pilot-backend-1.onrender.com/openapi.json

The `/chat` and `/tools/life_insurance_faq` responses are the ones this app
consumes; `matched_faq[]` entries include their full `answer` text, which
the "Related FAQs" expander renders.
